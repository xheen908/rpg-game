...
function GameContent({ 
  ...
  setGlobalTargetedId, currentMapId 
}) {
  ...
  return (
    <Suspense fallback={null}>
      ...
      {/* Map Auswahl basierend auf Lobby-State */}
      {currentMapId === 'MAP1' ? (
        <Map1 />
      ) : (
...
export default function TexturedArena() {
  ...
  // Synchronisation der Map-Wahl über Socket
  useEffect(() => {
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      if (e.code === 'Enter' && gameState === 'PLAYING' && !isPaused) setIsChatOpen(true);
    };
